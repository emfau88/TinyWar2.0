...
- HTML-Stand 2026-05-07: Top-Advance-Banner nutzt nun die segmentierten Originalframes aus `large ribbons` exakt in Original-Reihenfolge: Blue `0/1/3`, Red `10/12/13`. Die flexiblen Blue/Red-Koerper treffen in der Mitte direkt aufeinander; Abschluss-/Zwischenstuecke liegen nur an den aeusseren Seiten. Banner-Zahlen sind weiss wie im Original.
- HTML-Stand 2026-05-07: Linke Unit-Leiste und rechte Strategie-Leiste nutzen blaue `small ribbons`-Traeger im Originalmuster `[0,2,2,2,9]`, rotiert wie im Bevy-UI. Boost-Leiste bleibt offen, weil Boost-UI noch nicht als eigene HTML-Fläche existiert.
...
- HTML-Stand 2026-05-08: `Guard`-Effekt umgesetzt. Guard-faehige Units im aktuellen HTML-Scope sind Warriors; wenn sie unter aktiver `Guard`-Strategie angegriffen werden, gehen sie in Guard-Haltung, greifen nicht an und erhalten den originalnahen Defense-Bonus aus dem Original-Combat-Code. Wichtiger Original-Befund: Die Strategiebeschreibung spricht von `+50%` Armor/Magic-Resist, der eigentliche Original-Combat-Code verdoppelt diese Werte. Der HTML-Slice folgt hier bewusst dem Original-Codepfad statt nur dem Beschreibungstext. Red-AI nutzt vorerst weiter `Attack`.
...
- HTML-Stand 2026-05-08: Pause/Game-Speed fuer den Solo-Slice umgesetzt. `Space` toggelt Pause, `Ctrl+Right` verdoppelt Speed, `Ctrl+Left` halbiert Speed. Grenzen wie im Original: `0.25x` bis `16x`. Queue, Strategy-Cooldown, Enemy-Queue, Movement, Combat, Projectiles und Map-Tile-Animationen laufen ueber einen skalierten Simulations-Delta-Pfad; Kamera-Input bleibt nutzbar. Ein kleiner HUD-Status unten links zeigt `Nx` bzw. `Nx - paused`.
- HTML-Stand 2026-05-08: Unit-Info-Panel fuer den aktuellen Basic-Unit-Scope umgesetzt. Taste `H` toggelt das Overlay wie im Original-README; fuer Touch/Mobile existiert zusaetzlich ein kleiner `H`-Button oben rechts. Das Panel bleibt im HUD-Screen-Space, zoomt nicht mit der World-Camera und nutzt ein responsives Layout: auf breiten Viewports mit linker Unit-Liste, auf schmalen Viewports mit horizontaler Unit-Leiste oben. Angezeigt werden Original-Beschreibungstexte fuer Warrior/Lancer/Archer/Priest plus die bereits portierten HTML-Werte fuer Health, Damage/Healing, Cycle-Speed, Armor, Resist, Pen, Range und Spawn-Dauer.
